...
특히 3단계(Gem 팩토리)는 실제 제공된 개론서 [data/astro.pdf](file:///Users/pileuszu/Repos/bist-mini-2/data/astro.pdf)의 내용(우주론, 암흑물질 27%, 암흑에너지 68%, 우주배경복사 등)에 기반하여 최신 천문학 학술 논문 RAG와 융합 비평하는 극적인 시나리오로 정밀 개정되었습니다.
...
*   **발표 대본**:
    ...
    > 
    > 오늘 시연은 실제 연구원의 업무 흐름에 맞춰, **[1단계. 실시간 듀얼 RAG 일반 채팅 허브 ➡️ 2단계. 비동기 배치 기반 대규모 문헌 분석기 ➡️ 3단계. 천문학 개론서 PDF를 연동하는 사용자 정의 Gem 비서]** 순서로 진행하겠습니다. 각 단계는 약 5분씩 진행되며 플랫폼의 아키텍처적 우수성과 비즈니스 가치를 단계별로 소상히 입증해 보이겠습니다. 그럼 시연을 시작하겠습니다."
...
*   **발표 대본**:
    ...
    > 연구원은 신기술 연구 초입에서 보통 '이 기술의 검증된 학술적 원리는 무엇인가?', 그리고 '현재 시장이나 산업계에서의 실시간 동향은 어떠한가?'라는 두 가지 상호보완적 관점을 동시에 탐색하길 원합니다.
    > 
    > 기존의 에이전트 시스템은 질문의 카테고리를 LLM이 먼저 분류한 뒤 분류된 단일 노드로만 라우팅하는 방식을 취했습니다. 이 방식은 분류 노드에서 약 1초의 시간 지연이 발생할 뿐 아니라, 논문 정보나 웹 정보 중 한쪽에 치우친 답변만을 내놓는 정보 편향이 있었습니다. 
...
*   **발표 대본**:
    ...
    > 답변 내용을 보시면, 학계에서 입증된 CRISPR의 분자생물학적 원리 및 오프타겟 발생 기전(논문 RAG 지식)과 현재 글로벌 제약사들이 진행 중인 최근 임상 3상 진행 뉴스(실시간 Web 지식)가 문단 안에서 유기적으로 크로스 레퍼런스(Cross-Reference)되어 기술되어 있습니다.
    > 
...
*   **시연 목적**: 페르소나 주입을 통한 나만의 비서 개설, PDF 업로드 시 800자 청킹 및 격리 pgvector 동적 컬렉션 빌드, `context_schema` 및 `state_schema` 기반 병렬 RAG 동시 툴 구동, 삭제 시 물리 데이터 완전 파쇄(Wipe-Out)를 검증합니다. 특히 `data/astro.pdf`에 기록된 천문학 개론 데이터(CMB, 암흑물질 27%, 암흑에너지 68% 스펙)를 활용하여 최신 학술 논문 RAG와 대조 검증을 수행합니다.
...
*   **발표 대본**:
    > "(1분 - 기능 및 배경 설명)
    > 마지막으로, 학술 지식 검색의 한계를 완전히 초월하여 연구원 본인의 개별 참고 서적이나 보고서를 가르치고 맞춤형 페르소나를 조립하는 **Gem 팩토리**를 시연하겠습니다.
    > 
    > 플랫폼이 기본으로 제공하는 공인 논문 데이터 외에, 교육 현장이나 개별 연구팀이 보유한 천문학 개론서 같은 로컬 문서([astro.pdf](file:///Users/pileuszu/Repos/bist-mini-2/data/astro.pdf))를 주입하고, AI에게 특정한 역할(예: 천문학 튜터, 논문 비평가)을 부여해 최신 연구 트렌드와 비교하는 지식 융합 시나리오를 구성해 보겠습니다."

*   **발표자 행동**: `Create Gem` 버튼을 누르고 아래의 내용을 기입해 생성합니다.
    * **Gem 이름**: `"AI Astronomy Tutor (천문학 학습 비서)"`
    * **RAG 분야 필터**: `Astronomy (astronomy)` 선택
    * **시스템 프롬프트 (페르소나)**: `"당신은 학술 지식 기반의 천문학 교육 튜터입니다. 사용자가 입력한 개론서 파일의 기초 개념 설명과 실제 astronomy 학술 DB(학술 연구 논문)의 최신 발견 사항들을 비교하여, 기초 서적의 내용이 최신 학술 트렌드와 어떻게 대조되고 보완되는지 명확한 비교 분석 표와 함께 깊이 있는 피드백을 마크다운으로 제공하세요."`
*   **발표 대본**:
    > "(1분 30초 - 페르소나 주입 및 파일 RAG 격리 설명)
    > 비서 이름은 천문학 학습 비서, 참고 도메인은 astronomy로 좁혀 지정하였고, 개론 지식과 실제 천문학 최신 논문을 대조 분석하여 마크다운 표로 깔끔하게 정리하라는 특화 페르소나를 주입해 Gem을 개설했습니다.
    > 
    > 이제 이 비서에게 천문학 입문 개론서인 [data/astro.pdf](file:///Users/pileuszu/Repos/bist-mini-2/data/astro.pdf) 파일을 직접 업로드하겠습니다.
    > 
    > 업로드 버튼을 누르면 백엔드에서 즉시 PDF 텍스트를 파싱하여 800자 크기로 쪼개고, `text-embedding-3-large` 모델을 사용하여 3072차원의 고차원 벡터로 변환합니다.
    > 
...
*   **발표자 행동**: [data/astro.pdf](file:///Users/pileuszu/Repos/bist-mini-2/data/astro.pdf) 파일을 파일 업로드 영역에 드래그 앤 드롭한 뒤, 우측 대화창을 열고 아래의 복합 질문을 전송하여 툴 구동 배지와 출력되는 표 답변을 렌더링합니다.
    > **[입력 질문]**: `"내가 업로드한 astro.pdf의 8장에 나와 있는 '암흑 물질(Dark Matter)과 암흑 에너지(Dark Energy)의 우주 구성 비율(27% 및 68%)' 및 '우주배경복사(CMB)'의 설명에 대해, 최근 astronomy 학술 논문들(예: 제임스 웹이나 최신 우주론 연구)이 밝혀내고 있는 우주 가속 팽창과 암흑 기여도 최신 관측 트렌드와 어떻게 대조되고 상호 보완되는지 마크다운 표로 정리해서 피드백해줘."`
*   **발표 대본**:
    > "(1분 30초 - 2-트랙 병렬 RAG 및 대화 제어 구동 설명)
    > 업로드한 입문용 `astro.pdf`에 기술된 고전적 우주 구성비(암흑물질 27%, 암흑에너지 68%)와 실제 학술 DB 상의 최신 우주론 연구 트렌드를 대조하라는 복합 질문을 보냈습니다.
    > 
    ...
    > 
    > 런타임에 에이전트는 `_make_file_search_tool` 클로저 팩토리를 통해 `gem_id`가 내장된 격리 파일 RAG 도구를 동적으로 만들어 묶고, 대화 턴 간 수많은 검색 소스가 가산 누적될 때 컨텍스트가 폭발하지 않도록 `reduce_sources` 리듀서 상태 함수가 중복을 걸러내고 매 대화 턴의 출처 데이터를 깔끔하게 누적 업데이트합니다.
    > 
    > 화면을 보십시오. 업로드된 `astro.pdf` 8장의 기초적인 우주배경복사(CMB) 및 암흑 물질 27%, 암흑 에너지 68%라는 정적인 정보와, 최근 제임스 웹 우주망원경이나 초신성 관측을 통해 논쟁 중인 암흑에너지의 시간 가변성 등 최신 학술 논문 팩트들이 마크다운 표로 선명하게 비교 매칭되어 출력되었습니다. 
    > 사용자가 등록한 튜터 페르소나 지침에 따라 초보자도 이해하기 쉬우면서도 학술적으로 명확한 평가 코멘트가 완성되었습니다."
...
*   **발표자 대본**:
    ...
    > 
    > 삭제가 수행되는 즉시 PostgreSQL의 외래키 `ON DELETE CASCADE` 규칙에 의해 메타데이터와 대화 히스토리 데이터가 자동 연쇄 소거되며, 백엔드에서는 pgvector 엔진에 `adelete_collection(gem_id)` 비동기 명령을 전달하여, 물리 디스크 상에 동적 개설되어 있던 격리 임베딩 공간 자체를 흔적도 없이 **완전 파쇄(Wipe-Out)**합니다. 
