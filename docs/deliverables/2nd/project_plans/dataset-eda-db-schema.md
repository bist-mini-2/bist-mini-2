...
### 1.5 각 도메인별 2만 건 이내의 경량 최적화 후보군 설계
대규모 빌드 전, 개발 및 RAG 테스트 가속화를 위해 **각 도메인별로 데이터 규모가 약 2만 건 이하인 세부 카테고리 1개씩만 매핑**하여 경량 데이터셋(Lightweight Dataset)을 구축하는 최적화 안입니다.

| 학술 도메인 | 추천 서브 카테고리 | 서브 카테고리 의미 및 RAG 가치 | 데이터 개수 (건수) |
| :--- | :--- | :--- | :--- |
| **컴퓨터 과학 (CS)** | `cs.NE` (또는 `cs.DB`) | Neural and Evolutionary Computing (신경망 연산) <br>*(또는 Databases, 11,845건)* | **17,825 개** |
| **생명공학 (Bio)** | `q-bio.QM` (또는 `q-bio.BM`) | Quantitative Methods (정량 분석 방법론) <br>*(또는 Biomolecules, 6,754건)* | **13,229 개** |
| **천문학 (Astronomy)** | `astro-ph.EP` | Earth and Planetary Astrophysics (지구 및 행성 천체물리)<br>*천문학 카테고리 중 최소 규모* | **35,083 개** |

*   **최적화 의의**:
    - 위와 같이 경량 최적화 후보군(`cs.NE` + `q-bio.QM` + `astro-ph.EP`)으로만 RAG 데이터베이스를 구축할 경우, 전체 대상 논문 수는 **66,137 개**로 극적으로 축소됩니다.
    - 이는 기존 전체 3대 도메인(138.2만 건) 대비 **약 4.7% 수준**에 불과합니다.
    - M4 로컬 직접 배치 임베딩 연산 시, **단 3분(188초) 이내**에 전체 RAG 인프라 적재를 성공적으로 완료할 수 있어, 초고속 개발 및 테스트가 가능합니다.
    - OpenAI `text-embedding-3-large` 실시간 API 표준 요금으로 변환하더라도 **단 $1.72 (한화 약 2,300원)** 내외로 전체 벡터 DB 빌드가 가능합니다.
