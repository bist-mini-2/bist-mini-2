...
### 1.5 각 도메인별 5,000건 샘플링을 통한 초경량 최적화 후보군 설계
대규모 빌드 전, 개발 속도 극대화 및 RAG 성능 검증의 가속화를 위해 **각 도메인별로 가장 RAG 시맨틱 가치가 높은 대표 세부 카테고리를 선정하고, 각 카테고리당 5,000건씩 랜덤 샘플링**하여 총 15,000건의 초경량 데이터셋(Super Lightweight Dataset)을 구축합니다.

| 학술 도메인 | 대상 서브 카테고리 | 서브 카테고리 의미 및 RAG 가치 | 전체 건수 | 추출 건수 (샘플링) |
| :--- | :--- | :--- | :---: | :---: |
| **컴퓨터 과학 (CS)** | `cs.NE` | Neural and Evolutionary Computing (신경망 및 진화 연산) <br>*AI 에이전트 RAG 질문 테스트에 적합한 주제* | 17,825 건 | **5,000 건** |
| **생명공학 (Bio)** | `q-bio.BM` | Biomolecules (분자생물학, 단백질 구조) <br>*전문 학술 도메인의 지식 밀도가 높은 핵심 분야* | 6,754 건 | **5,000 건** |
| **천문학 (Astronomy)** | `astro-ph.EP` | Earth and Planetary Astrophysics (지구 및 행성 천체물리) <br>*천문학 분야에서 가장 작고 대중적인 시맨틱 질문 가능* | 35,083 건 | **5,000 건** |
| **합계** | - | - | 59,662 건 | **15,000 건** |

*   **최적화 의의**:
    - **초경량 데이터 인프라**: 위와 같이 도메인당 5,000건씩 총 **15,000건**의 논문으로 구성할 경우, 전체 3대 도메인 원본(138.2만 건) 대비 **약 1.08%** 수준으로 데이터를 경량화할 수 있습니다.
