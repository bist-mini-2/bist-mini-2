...
### 1.5 단일 대표 카테고리 필터링 시 데이터셋 규모 (1순위 카테고리 기준)
전수 조사를 통한 각 도메인별 최다(1순위) 서브 카테고리를 기준으로, 데이터셋 후보군을 도메인별로 딱 1개의 대표 카테고리만 가져간다고 가정했을 때의 데이터 규모 통계입니다.

| 학술 도메인 | 대표 1순위 카테고리 | 의미 및 대상 기술 범위 | 데이터 개수 (건수) |
| :--- | :--- | :--- | :--- |
| **컴퓨터 과학 (CS)** | `cs.LG` | Machine Learning (기계 학습 및 딥러닝 핵심) | **272,646 개** |
| **생명공학 (Biotechnology)** | `q-bio.QM` | Quantitative Methods (정량 분석 및 전산 생물학) | **13,229 개** |
| **천문학 (Astronomy)** | `astro-ph.GA` | Astrophysics of Galaxies (은하 천체물리학) | **77,743 개** |
| **천문학 (Astro-Legacy)** | `astro-ph` | Astrophysics (Legacy 통합 카테고리) | *105,380 개* |

*   **최적화 의의**:
    - 이와 같이 도메인별로 딱 1개의 대표 카테고리만 선정하여 RAG 데이터셋을 구축할 경우, 전체 대상 논문 수는 **363,618 개** (CS: cs.LG, Bio: q-bio.QM, Astro: astro-ph.GA 기준)로 축소됩니다.
    - 이는 기존 3대 도메인 전체 필터링 수(138.2만 건) 대비 **약 26% 수준**으로 감소된 규모입니다.
    - M4 로컬 직접 배치 임베딩 연산 시, **단 17분 내외**로 3대 도메인의 대표 데이터를 완벽히 변환하여 적재할 수 있어 개발 및 테스트 사이클이 획기적으로 가속화됩니다.
