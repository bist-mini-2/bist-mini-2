...
| :---: | :--- | :--- | :---: |
| **W-01** | 유저 메인 채팅 화면 | [user-main-chat.html](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/user-main-chat.html) | [Screenshot](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/images/user-main-chat.png) |
| **W-02** | 문헌 요약 리포트 화면 | [user-report-view.html](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/user-report-view.html) | [Screenshot](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/images/user-report-view.png) |
| **W-03** | 관리자/검증용 테스트 화면 | [admin-test.html](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/admin-test.html) | [Screenshot](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/images/admin-test.png) |
| **W-04** | 문헌 보관함 및 리포트 아카이브 화면 | [user-library-archive.html](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/user-library-archive.html) | [Screenshot](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/images/user-library-archive.png) |
| **W-05** | 인용 관계 그래프 및 상세 화면 | [user-citation-graph.html](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/user-citation-graph.html) | [Screenshot](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/images/user-citation-graph.png) |
| **W-06** | 개인용 보안 샌드박스 설정 화면 | [user-sandbox-control.html](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/user-sandbox-control.html) | [Screenshot](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/images/user-sandbox-control.png) |
| **W-07** | 유저 피어 리뷰 워크숍 화면 | [user-peer-review.html](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/user-peer-review.html) | [Screenshot](file:///c:/Repo/bist-mini-2-backend/docs/wireframes/images/user-peer-review.png) |
...
*   **MTEB 벌크 데이터 로더** (`c:/Repo/bist-mini-2-backend/scripts/data_loader.py`):
    *   *역할*: 의학(NFCorpus), 컴퓨터과학(SCIDOCS), 자연과학(SciFact) 데이터를 다운로드 및 파싱하여 DB에 업로드하는 자동화 스크립트.
*   **텍스트 청커 및 전처리기** (`c:/Repo/bist-mini-2-backend/scripts/chunker.py`):
    *   *역할*: 문단 분실 없이 500자 단위로 논문을 절단하고 메타데이터와 벡터 임베딩을 결합하는 가공 유틸리티.

### 🔌 2.3 단계 2: 백엔드 API & DTO 정의 (FastAPI Layer)
*   **API 핸들러 및 라우터** (`c:/Repo/bist-mini-2-backend/app/routers/`):
    *   *역할*: RAG 유사도 검색(`POST /similarity-search/*`), 격리 파일 전송(`POST /validation/upload-isolated`), 요약 보고서 등록(`POST /library/archive`) 등의 엔드포인트 구현.
*   **Pydantic DTO 스키마** (`c:/Repo/bist-mini-2-backend/app/schemas/`):
    *   *역할*: 인용 정보 DTO(`CitationSource`), 가설 검증 결과 DTO, 채점 스코어카드 및 리포트 내보내기용 DTO 정의.
*   **보안 가드 및 파일 와이퍼** (`c:/Repo/bist-mini-2-backend/app/utils/security.py`):
    *   *역할*: Directory Traversal 방지 경로 가드 및 30분 초과 시 비활성 세션을 파쇄(Wipe)하는 데몬 로직.

### 🧠 2.4 단계 3: AI 에이전트 & LangGraph 구축 (Agent Layer)
*   **에이전트 오케스트레이터 및 그래프** (`c:/Repo/bist-mini-2-backend/app/agents/graph.py`):
...
*   **프롬프트 템플릿 매니저** (`c:/Repo/bist-mini-2-backend/app/agents/prompts.py`):
    *   *역할*: Step-Back 질문 변환, CoT 추론 유도, 학술 피어 리뷰(방법론/신규성/문체) 3대 리뷰어 전용 프롬프트 리포지토리.
*   **요약 미들웨어 및 영구 메모리** (`c:/Repo/bist-mini-2-backend/app/utils/memory.py`):
    *   *역할*: `SummarizationMiddleware` 토큰 최적화 및 `PostgresSaver` 데이터베이스 세션 관리 모듈.

### 💻 2.5 단계 4: 프론트엔드 연동 (Frontend & API Integration)
*   **Next.js API 클라이언트** (`c:/Repo/bist-mini-2-backend/frontend/services/api.js`):
    *   *역할*: 백엔드 FastAPI 엔드포인트를 호출하여 스레드, 샌드박스, 피어 리뷰 정보를 수신하는 fetch 모듈.
*   **SSE 토큰/CoT 스트리밍 리스너** (`c:/Repo/bist-mini-2-backend/frontend/hooks/useStream.js`):
    *   *역할*: 실시간 응답 토큰 및 생각의 흐름 로그를 실시간으로 스트리밍하여 W-01 화면에 점진적으로 렌더링하는 React Hook.
*   **D3/인터랙티브 그래프 렌더러** (`c:/Repo/bist-mini-2-backend/frontend/components/GraphRenderer.js`):
