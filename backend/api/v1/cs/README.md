...
- [endpoints.py](file:///c:/Repo/bist-mini-2/backend/api/v1/cs/endpoints.py): pgvector 코사인 유사도 거리를 활용한 RAG 기반 논문 유사도 검색 API (`POST /similarity-search/cs`), LLM 답변 생성 API (`POST /similarity-search/cs/ask`), 그리고 RAG 툴 바인딩 에이전트 API (`POST /similarity-search/cs/agent`)의 엔드포인트 구현부입니다.
- [services.py](file:///c:/Repo/bist-mini-2/backend/api/v1/cs/services.py): RAG 유사도 검색의 임베딩 변환, 매핑, RAG 답변 생성 및 Langsmith/LangGraph 기반 툴 호출 에이전트 구동 비즈니스 로직을 처리하는 서비스 레이어 클래스입니다.
...
- [models.py](file:///c:/Repo/bist-mini-2/backend/api/v1/cs/models.py): 요청 및 응답 검증을 위한 Pydantic DTO 정의 파일입니다. (RAG 생성용 DTO들과 에이전트용 `CsAgentQueryRequest`, `CsAgentQueryResponse` 추가)
