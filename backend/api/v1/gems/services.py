...
class GemService:
    ...
    async def upload_files(
        ...
        Args:
            member_id (str): 요청 사용자 ID.
            gem_id (str): 파일을 연결할 Gem ID.
            files (list[tuple[str, bytes]]): (파일명, 바이너리) 튜플 목록.

        Returns:
            dict: 처리된 파일 수와 총 청크 수.
        """
        gem = await self._get_owned_gem(member_id, gem_id)
        ...
        # ① has_files 먼저 커밋 대상에 올린 뒤 flush — gem_file insert 실패와 무관하게 보존
    ...
    async def list_files(self, member_id: str, gem_id: str) -> list[GemFileEntity]:
        """Gem에 업로드된 파일 메타데이터 목록을 반환한다. 소유자만 가능.
    ...
    async def send_message_stream(
        ...
        """Gem에 메시지를 보내 스트리밍으로 토큰 단위 답변을 받는다.
        ...
        Yields:
            str: JSON 직렬화된 이벤트 줄 (type: token|status).
        """
        gem = await self._get_owned_gem(member_id, gem_id)
        db_sources = gem.db_sources.split(",")
