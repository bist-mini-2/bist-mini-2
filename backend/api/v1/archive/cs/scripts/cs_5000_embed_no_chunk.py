...
# 백업용 JSONL 저장 경로
OUTPUT_FILE_PATH_DATA = os.path.abspath(os.path.join(ROOT_DIR, "data/raw/archive/local_embeddings_no_chunk_output.jsonl"))
...
async def save_to_db(collected_papers: list) -> None:
    """수집된 논문 메타데이터와 임베딩 데이터를 DB의 cs_collections 및 cs_embeddings 테이블에 비동기로 저장합니다.
    적재 전 기존 임베딩 테이블 데이터를 완전히 비웁니다(Truncate/Delete).
    """
    print(f"\n{BOLD}[3/4] 🗄️ PostgreSQL (pgvector) 데이터베이스 적재 시작...{RESET}")
    start_time = time.time()
    
    async with session_maker() as session:
        # 1. 기존 임베딩 데이터 비우기 (Truncate/Delete)
        print(f"  > {YELLOW}기존 임베딩 테이블(cs_embeddings) 데이터 비우는 중...{RESET}")
        await session.execute(delete(CsEmbeddingEntity))
        await session.commit()
        print(f"  {GREEN}✓ 기존 데이터 비우기 완료!{RESET}")

        # 2. cs.NE 컬렉션 가져오거나 생성
        ...
        # 3. 신규 저장할 CsEmbeddingEntity 목록 구성
        ...
        if embeddings_to_insert:
            print(f"  > {CYAN}신규 데이터 적재:{RESET} 임베딩 {len(embeddings_to_insert):,}건")
...
async def main() -> None:
    """CS.NE 도메인 논문을 수집하고 청킹 없이 통째로 OpenAI 임베딩 API를 통해 벡터화하여 저장 및 DB 업로드를 진행하는 메인 함수."""
    ...
    print(f"\n{BOLD}======================================================================{RESET}")
    print(f"{BOLD}{GREEN}🚀 [START] cs.NE 학술 논문 {TARGET_COUNT}건 임베딩 & DB 적재 파이프라인 (청킹 없음){RESET}")
    print(f"{BOLD}======================================================================{RESET}")
    
    print(f"\n{BOLD}[1/4] 📄 arxiv-metadata-oai-snapshot.json 로드 시작 (청킹 안함)...{RESET}")
    
    collected_papers = []
    start_time = time.time()

    # 1. 데이터 수집 (초록 청킹 없이 통째로 리스트 구성)
        ...
        for line in infile:
            ...
            try:
                ...
                collected_papers.append({
                    ...
                    "chunk_texts": [abstract]  # 청킹 없이 초록 전체를 1개의 청크로 지정
                })
    ...
    # 2. 모든 청크들을 평탄화(Flatten)하여 배치 임베딩 연산용 리스트 구성 (청크 인덱스는 무조건 0)
    ...
    print(f"\n{BOLD}[2/4] 🧠 OpenAI 배치 임베딩 연산 시작 (전체 초록)...{RESET}")
    print(f"  > 총 임베딩 개수: {total_chunks:,}개 (모델: {MODEL_NAME}, 3,072차원)")
    ...
    for i in range(0, total_chunks, BATCH_SIZE):
        ...
        if (i + BATCH_SIZE) % 500 == 0 or (i + BATCH_SIZE) >= total_chunks:
            ...
            print(f"  > 임베딩 진행: {current_processed:,} / {total_chunks:,} 초록... (속도: {rate:.1f} items/sec)")
    ...
    # 4. JSON Line 형태로 로컬 캐시 파일 저장 (백업 저장)
    print(f"\n{BOLD}[3/4] 💾 백업용 로컬 캐시 JSONL 파일 저장 중...{RESET}")
    print(f"  > 파일 경로: {OUTPUT_FILE_PATH_DATA}")
    with open(OUTPUT_FILE_PATH_DATA, "w", encoding="utf-8") as outfile:
        for paper in collected_papers:
            # chunk_texts 키 제거 후 chunks (벡터 포함) 리스트 저장
            paper_copy = paper.copy()
            if "chunk_texts" in paper_copy:
                del paper_copy["chunk_texts"]
            outfile.write(json.dumps(paper_copy, ensure_ascii=False) + "\n")
                
    print(f"  {GREEN}✓ 백업 파일 저장 완료!{RESET}")
    ...
    print(f"\n{BOLD}======================================================================{RESET}")
    print(f"{BOLD}{GREEN}🎉 [SUCCESS] RAG 임베딩 및 DB 업로드 완료! (중복 제거 & 청킹 없음){RESET}")
