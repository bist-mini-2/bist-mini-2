...
def test_similarity_search_cs_endpoint(mock_search_papers):
    ...
    # Mock 응답 데이터 설정
    mock_search_papers.return_value = SimilaritySearchResponse(
        results=[
            SimilaritySearchResult(
                doc_id="2406.12345",
...
def test_ask_cs_rag_endpoint(mock_ask_rag):
    ...
    # Mock 응답 데이터 설정
    mock_ask_rag.return_value = CsRagQueryResponse(
        answer="모킹된 답변 결과입니다.",
        sources=[
            SimilaritySearchResult(
                doc_id="2406.12345",
...
def test_ask_cs_agent_endpoint(mock_run_agent):
    ...
    # Mock 응답 데이터 설정
    mock_run_agent.return_value = CsAgentQueryResponse(
...
async def test_cs_service_business_logic():
    ...
    # Mock DAO 생성
    mock_dao = MagicMock()
    mock_dao.select_similar_chunks = AsyncMock(return_value=[
        ("doc-101", "Title A", "Chunk text content", 0.88)
    ])
