...
- **RAG 텍스트 청킹(Chunking) 표준 규격**: 논문 초록은 별도의 청킹 분할을 거치지 않고, 각 논문의 초록(Abstract) 전체를 단일 임베딩 벡터로 일괄 업로드하여 관리합니다.
- **임베딩 벡터 차원**: 임베딩 차원은 **3072차원**을 표준으로 채택하며, text-embedding-3-large 모델을 활용하여 pgvector 테이블의 `bio_embeddings`, `cs_embeddings`, `astronomy_embeddings` 컬렉션에 적재할 때 vector(3072) 규격을 준수합니다.
...
- 각 학술 도메인(bio, cs, astronomy 등)의 서비스 로직은 Git Merge Conflict 방지를 위해 도메인별 폴더 내에 격리하여 구현하는 것을 원칙으로 합니다.
- 단, RAG 핵심 검색 파이프라인과 관련 에이전트 도구(tools)의 경우 일관된 RAG 성능 및 테이블 공유 관리를 위해 공통 모듈인 `backend/api/common/rag_pipeline.py`에 단일화하여 공유합니다. 이 파일은 외부 API 엔드포인트를 노출하지 않고, 개별 기능(gems, chat, research_gap 등)에서 파이썬 모듈 형태로 호출하여 재사용하거나 에이전트 도구로 바인딩하여 사용합니다.

### 13. 도메인 내부 모듈(서비스, 핸들러, 툴) 물리적 격리
- 각 학술 도메인 폴더(예: `backend/api/v1/[도메인]/`) 내부에서 서비스 로직의 순수성을 보존하고 관심사를 격리하기 위해 다음과 같이 파일을 물리적으로 분리합니다.
  - **`services.py`**: 순수 비즈니스 서비스 로직 및 DB 쿼리 연산만 담당합니다.
  - **`handler.py`**: LLM의 프롬프트 입출력 로깅을 처리하는 `BaseCallbackHandler` 상속 콜백 핸들러 클래스 등 이벤트/로깅 핸들러를 별도 정의합니다.
  - **`tools.py`**: LangChain/LangGraph 에이전트가 호출하는 @tool 데코레이터 기반 에이전트 도구들을 정의합니다. 단, 해당 도메인이 공통 RAG 검색 툴만 사용할 경우, 빈 래퍼 `tools.py` 파일을 생성하지 않고 `api.common.rag_pipeline`에서 직접 임포트하여 에이전트에 바인딩합니다.
- **순환 참조 방지**: 에이전트 툴(`tools.py`)과 비즈니스 서비스(`services.py`) 간의 상호 참조로 인한 순환 참조를 차단하기 위해 `from typing import TYPE_CHECKING` 구문 및 `if TYPE_CHECKING:` 구절 안에서 타입 힌트용 임포트를 수행합니다.

### 14. Hugging Face 데이터셋 관리 및 백업 정책 (Dataset Management & Backup)
- **Parquet 포맷 활용**: RAG 임베딩 등 대용량 벡터 데이터셋 백업 및 버전 관리 시 CSV/JSON 대신 메모리 효율성, 압축률, 스키마 무결성이 높은 **Parquet** 포맷 사용을 의무화합니다.
- **분할 쿼리(Paging) 및 네트워크 교착 방지**: 데이터베이스로부터 대용량 벡터 데이터를 내보낼 시 단일 쿼리 호출로 인한 원격 연결 끊김 및 네트워크 hang(커넥션 락업)을 방지하기 위해, `LIMIT`와 `OFFSET`을 이용한 **페이징(Paging, 예: 5,000건 단위) 처리**를 필수로 설계합니다.
- **Dataset Dict 스펙 및 Metadata**: 여러 도메인의 임베딩 데이터를 단일 레포지토리에 저장할 때는 Hugging Face의 `DatasetDict` 형태를 유지하고 도메인별 Subset(예: `bio`, `cs`, `astronomy`)으로 구분하여 관리합니다.
- **Dataset Card (README.md) 작성**: 데이터 업로드 시 원본 데이터 출처(Kaggle 등)와 데이터 가공 모델(`text-embedding-3-large` 등, 차원 정보 3072), 그리고 변환 목적 및 데이터 스키마 설명을 담은 `README.md` Dataset Card 생성을 자동화하고 리포지토리에 함께 업로드되도록 합니다.
