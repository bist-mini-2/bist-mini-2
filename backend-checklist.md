...
- [ ] **일관된 응답 구조**: 모든 HTTP API 응답이 표준 JSON 구조를 따르고 있습니까?
  ...
  - **예외 사항 (인증 API)**: OAuth2.0 표준 규격(RFC 6749)을 활용하는 Swagger UI의 `Authorize` 자물쇠 인증 기능과의 호환성을 보장하기 위해, 로그인/토큰 발급 엔드포인트(예: `/auth/login`)에 한해서는 응답 성공 래퍼를 씌우지 않고 루트 레벨에 `access_token`과 `token_type`이 담긴 플랫 JSON 구조를 직접 반환합니까?
- [ ] **HTTP 상태 코드 매핑 규칙**:
